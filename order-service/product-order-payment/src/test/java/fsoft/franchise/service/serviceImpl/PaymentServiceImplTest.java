...
class PaymentServiceImplTest {
        ...
        @Mock
        private fsoft.franchise.service.OrderTrackingService orderTrackingService;
        ...
        void setUp() {
                ...
                // vnpResponseCode and vnpBankCode now live on PaymentEntity
        ...
        class GetPaymentsTests {
                ...
                void shouldReturnPaymentsForFranchiseAdmin() {
                        ...
                        // When
                        PaymentListResponse result = paymentService.getPayments(filterRequest, customerId,
                                        "FRANCHISE_ADMIN");
                ...
                void shouldReturnPaymentsForCustomer() {
                        ...
                        verify(paymentRepository).findByCustomerFilters(eq(customerId), any(), any(), any(), any(),
                                        any());
                }
                ...
                void shouldIncludeErrorMessageForFranchiseAdmin() {
                        // Given
                        payment.setVnpResponseCode("99"); // Failed — response code now lives on PaymentEntity
                        Page<PaymentEntity> page = new PageImpl<>(List.of(payment));
                        when(paymentRepository.findByAdminFilters(any(), any(), any(), any(), any(), any(), any()))
                                        .thenReturn(page);
                        when(transactionRepository.findByPayment_IdOrderByCreatedAtDesc(paymentId))
                ...
                void shouldRecognizeStoreManagerAsAdminRole() {
                        ...
                        verify(paymentRepository).findByAdminFilters(any(), any(), any(), any(), any(), any(), any());
                        verify(paymentRepository, never()).findByCustomerFilters(any(), any(), any(), any(), any(),
                                        any());
                }
        }
        ...
        class GetPaymentStatusTests {
                ...
                void shouldReturnPaymentStatusForAdmin() {
                        ...
                        // When
                        PaymentStatusResponse result = paymentService.getPaymentStatus(orderId, customerId,
                                        "FRANCHISE_ADMIN");
                ...
                void shouldAllowStoreManagerToViewAnyPayment() {
                        ...
                        // When
                        PaymentStatusResponse result = paymentService.getPaymentStatus(orderId, strangerId,
                                        "STORE_MANAGER");
        ...
        class CreatePaymentTests {
                ...
                void shouldCreatePaymentSuccessfully() {
                        ...
                        when(paymentRepository.existsByOrder_IdAndStatus(orderId, PaymentStatus.PAID))
                                        .thenReturn(false);
                        ...
                        // When
                        PaymentResponse result = paymentService.createPayment(createPaymentRequest, ipAddress);
                        ...
                        verify(paymentRepository).findFirstByOrder_IdAndStatus(orderId, PaymentStatus.PENDING);
                        verify(transactionRepository).existsByVnpTxnRef(anyString());
                        verify(paymentRepository, times(2)).save(any(PaymentEntity.class)); // initial + paymentUrl
                ...
                @Test
                @DisplayName("Should reject payment when order already has a PAID payment")
                void shouldRejectPaymentWhenOrderAlreadyPaid() {
                        ...
                        assertEquals(PaymentErrorCode.PAYMENT_ALREADY_PAID, exception.getErrorCode());
                        verify(paymentRepository, never()).save(any());
                        verify(transactionRepository, never()).save(any());
                }

                @Test
                @DisplayName("Should cancel expired PENDING payment and create new one on retry")
                void shouldCancelOldPendingPaymentOnRetry() {
                        // Given — old payment has EXPIRED (expiredAt in the past)
                        ...
                        when(transactionRepository.findByPayment_IdOrderByCreatedAtDesc(oldPayment.getId()))
                        ...
                        // Then
                        assertNotNull(result);
                        assertEquals(PaymentStatus.FAILED, oldPayment.getStatus());
                        verify(paymentRepository, atLeast(2)).save(any(PaymentEntity.class));
                }

                @Test
                @DisplayName("Should reject new payment when an active (unexpired) PENDING payment exists")
                void shouldRejectPaymentWhenActivePendingExists() {
                        // Given — old payment is still ACTIVE (expiredAt in the future)
                        PaymentEntity activePayment = PaymentEntity.builder()
                                        .id(UUID.randomUUID())
                                        .order(order)
                                        .status(PaymentStatus.PENDING)
                                        .paymentMethod(PaymentMethod.MOMO)
                                        .expiredAt(LocalDateTime.now().plusMinutes(10)) // STILL ACTIVE
                                        .build();
                        when(orderRepository.findById(orderId)).thenReturn(Optional.of(order));
                        when(paymentRepository.existsByOrder_IdAndStatus(orderId, PaymentStatus.PAID))
                                        .thenReturn(false);
                        ...
                        assertEquals(PaymentErrorCode.PAYMENT_PENDING_ACTIVE, exception.getErrorCode());
                        // Should NOT cancel the active payment
                        assertEquals(PaymentStatus.PENDING, activePayment.getStatus());
                        verify(paymentRepository, never()).save(any());
                        verify(transactionRepository, never()).save(any());
                }

                @Test
                @DisplayName("Should reuse existing payUrl when same-method active PENDING payment exists")
                void shouldReusePayUrlWhenSameMethodActivePending() {
                        // Given — old VNPAY payment is still ACTIVE with a stored payUrl
                        ...
                        PaymentEntity activePayment = PaymentEntity.builder()
                                        ...
                                        .order(order)
                                        .status(PaymentStatus.PENDING)
                                        .paymentMethod(PaymentMethod.VNPAY) // SAME as request
                                        .expiredAt(expiry)
                                        .paymentUrl("https://sandbox.vnpayment.vn/existing-link")
                                        .build();
                        activePayment.setCreatedAt(createdAt);
                        when(orderRepository.findById(orderId)).thenReturn(Optional.of(order));
                        when(paymentRepository.existsByOrder_IdAndStatus(orderId, PaymentStatus.PAID))
                                        .thenReturn(false);
                        when(paymentRepository.findFirstByOrder_IdAndStatus(orderId, PaymentStatus.PENDING))
                        ...
                        // When — same method retry (user closed tab, reopened)
                        PaymentResponse result = paymentService.createPayment(createPaymentRequest, ipAddress);

                        // Then — returns existing payUrl, no new payment/transaction created
                        assertNotNull(result);
                        assertEquals(oldPaymentId, result.paymentId());
                        assertEquals("https://sandbox.vnpayment.vn/existing-link", result.paymentUrl());
                        assertEquals("PENDING", result.paymentStatus());
                        assertEquals(PaymentStatus.PENDING, activePayment.getStatus());
                        verify(paymentRepository, never()).save(any());
                        verify(transactionRepository, never()).save(any());
                ...
                void shouldSavePaymentWithCorrectStatusAndExpiry() {
                        ...
                        org.mockito.ArgumentCaptor<PaymentEntity> paymentCaptor = org.mockito.ArgumentCaptor
                                        .forClass(PaymentEntity.class);
                ...
                void shouldCreateTransactionWithCorrectDetails() {
                        ...
                        org.mockito.ArgumentCaptor<TransactionEntity> txnCaptor = org.mockito.ArgumentCaptor
                        ...
                        // TransactionStatus is no longer on TransactionEntity — status is tracked on PaymentEntity
        ...
        class ProcessWebhookTests {
                ...
                private final String txnRef = "TXN-" + orderId;
                ...
                void shouldProcessSuccessfulWebhook() {
                        ...
                        doNothing().when(orderTrackingService).sendTrackingUpdate(any(), any(), any(), any());

                        // When
                        WebHookResponse result = paymentService.processWebhook(webhookParams);
                ...
                @Test
                @DisplayName("Should process code 24 (customer cancelled) and update payment to CANCELLED")
                void shouldProcessCode24AsCancelled() {
                        // Given
                        webhookParams.put("vnp_ResponseCode", "24"); // Customer cancelled
                        order.setStatus(OrderStatus.PROCESSING);
                        when(transactionRepository.findByVnpTxnRef(txnRef))
                                        .thenReturn(Optional.empty());
                        when(paymentRepository.findByTransactionId(txnRef))
                        ...
                        // Then — code 24 → CANCELLED (nông: khách chủ động hủy)
                        assertNotNull(result);
                        assertEquals("CANCELLED", result.getStatus());
                        assertNull(result.getTransactionId()); // Không tạo Transaction

                        org.mockito.ArgumentCaptor<OrderEntity> orderCaptor = org.mockito.ArgumentCaptor
                ...
                @Test
                @DisplayName("Should be idempotent: processWebhook does not duplicate a payment that is already PAID")
                void shouldBeIdempotentForCompletedWebhook() {
                        // Given - Payment already PAID (idempotency: webhook should still succeed
                        // gracefully)
                        ...
                        assertNotNull(result);
                        assertEquals(paymentId, result.getPaymentId());
                        // Order is already PAID — guard prevents double-update
                ...
                void shouldUpdateTransactionWithWebhookDetails() {
                        ...
                        // Then — transaction carries the gateway IDs; response code/bank live on payment
                        TransactionEntity capturedTxn = txnCaptor.getValue();
                        ...
                        // Response codes are now on PaymentEntity
                        org.mockito.ArgumentCaptor<PaymentEntity> payCaptor = org.mockito.ArgumentCaptor.forClass(PaymentEntity.class);
                        verify(paymentRepository).save(payCaptor.capture());
                        assertEquals("00", payCaptor.getValue().getVnpResponseCode());
                        assertEquals("NCB", payCaptor.getValue().getVnpBankCode());
                        assertEquals(TransactionType.PAYMENT, capturedTxn.getType());
                }
                ...
                @Test
                @DisplayName("Should accept locally-cancelled VNPay transaction when gateway says success and no other PAID payment")
                void shouldAcceptLocallyCancelledVNPayWhenNoOtherPaid() {
                        // Given - Payment was locally FAILED (cancelled by retry flow);
                        // TransactionStatus removed
                        payment.setStatus(PaymentStatus.FAILED); // locally cancelled
                        ...
                        // Then - Should accept the payment
                        assertNotNull(result);
                        assertEquals("PAID", result.getStatus());
                        verify(transactionRepository).save(any(TransactionEntity.class));
                        verify(paymentRepository).save(any(PaymentEntity.class));
                ...
                @Test
                @DisplayName("Should detect double payment when locally-cancelled VNPay transaction succeeds but order already paid")
                void shouldDetectDoublePaymentForLocallyCancelledVNPay() {
                        // Given - Payment was locally FAILED, but order was already paid by another
                        // method
                        ...
                        when(transactionRepository.findByVnpTxnRef(txnRef))
                                        .thenReturn(Optional.of(transaction));
                        when(paymentRepository.existsByOrder_IdAndStatus(orderId, PaymentStatus.PAID))
                                        .thenReturn(true); // Another payment already PAID
                        ...
                        // Then - Should return DUPLICATE_PAID, NOT update payment/order
                        assertNotNull(result);
                        assertEquals("DUPLICATE_PAID", result.getStatus());
                        verify(transactionRepository).save(any(TransactionEntity.class)); // Records txn for
                                                                                          // reconciliation
                        verify(paymentRepository, never()).save(any(PaymentEntity.class)); // Does NOT change payment
                }

                // ── Mới: test từng VNPay response code đặc biệt
                // ──────────────────────────────────

                @Test
                @DisplayName("[Code 07] SUSPICIOUS — payment set to PAID with requiresReview flag")
                void shouldSetPaidForCode07Suspicious() {
                        // Given
                        webhookParams.put("vnp_ResponseCode", "07");
                        order.setStatus(OrderStatus.PROCESSING);
                        when(transactionRepository.findByVnpTxnRef(txnRef)).thenReturn(Optional.empty());
                        when(paymentRepository.findByTransactionId(txnRef)).thenReturn(Optional.of(payment));

                        org.mockito.ArgumentCaptor<PaymentEntity> payCaptor = org.mockito.ArgumentCaptor
                                        .forClass(PaymentEntity.class);
                        when(paymentRepository.save(payCaptor.capture())).thenReturn(payment);
                        when(orderRepository.save(any(OrderEntity.class))).thenReturn(order);
                        when(transactionRepository.save(any(TransactionEntity.class))).thenReturn(transaction);
                        doNothing().when(orderTrackingService).sendTrackingUpdate(any(), any(), any(), any());
                        ...
                        // Then — tiền đã trừ nên vẫn PAID
                        assertEquals("PAID", result.getStatus());
                        assertEquals(PaymentStatus.PAID, payCaptor.getValue().getStatus());
                        assertEquals("07", payCaptor.getValue().getVnpResponseCode());
                }

                @Test
                @DisplayName("[Code 09] NOT_REGISTERED — payment set to FAILED")
                void shouldSetFailedForCode09() {
                        // Given
                        webhookParams.put("vnp_ResponseCode", "09");
                        order.setStatus(OrderStatus.PROCESSING);
                        when(transactionRepository.findByVnpTxnRef(txnRef)).thenReturn(Optional.empty());
                        when(paymentRepository.findByTransactionId(txnRef)).thenReturn(Optional.of(payment));
                        ...
                        assertEquals(PaymentStatus.FAILED, payCaptor.getValue().getStatus());
                        assertEquals("09", payCaptor.getValue().getVnpResponseCode());
                        verify(transactionRepository, never()).save(any(TransactionEntity.class));
                }

                @Test
                @DisplayName("[Code 11] EXPIRED — payment set to FAILED, order back to PENDING")
                void shouldSetFailedForCode11Expired() {
                        // Given
                        webhookParams.put("vnp_ResponseCode", "11");
                        payment.setStatus(PaymentStatus.PENDING);
                        order.setStatus(OrderStatus.PROCESSING);
                        when(transactionRepository.findByVnpTxnRef(txnRef)).thenReturn(Optional.empty());
                        when(paymentRepository.findByTransactionId(txnRef)).thenReturn(Optional.of(payment));
                        ...
                        // Then — expired → FAILED, order → PENDING (để retry)
                        assertEquals("FAILED", result.getStatus());
                        assertEquals(OrderStatus.PENDING, orderCaptor.getValue().getStatus());
                }

                @Test
                @DisplayName("[Code 99] UNKNOWN — payment set to FAILED")
                void shouldSetFailedForCode99() {
                        // Given
                        webhookParams.put("vnp_ResponseCode", "99");
                        order.setStatus(OrderStatus.PROCESSING);
                        when(transactionRepository.findByVnpTxnRef(txnRef)).thenReturn(Optional.empty());
                        when(paymentRepository.findByTransactionId(txnRef)).thenReturn(Optional.of(payment));
                        when(paymentRepository.save(any(PaymentEntity.class))).thenReturn(payment);
                ...
                @Test
                @DisplayName("[Unknown code] Fallback to UNKNOWN enum — payment set to FAILED")
                void shouldFallbackToUnknownForUnrecognizedCode() {
                        // Given - code không có trong enum
                        webhookParams.put("vnp_ResponseCode", "88");
                        order.setStatus(OrderStatus.PROCESSING);
                        when(transactionRepository.findByVnpTxnRef(txnRef)).thenReturn(Optional.empty());
                        when(paymentRepository.findByTransactionId(txnRef)).thenReturn(Optional.of(payment));
                        when(paymentRepository.save(any(PaymentEntity.class))).thenReturn(payment);
                        ...
                        // Then — không có enum → fallback UNKNOWN → FAILED
                        assertEquals("FAILED", result.getStatus());
                }
        }
}
