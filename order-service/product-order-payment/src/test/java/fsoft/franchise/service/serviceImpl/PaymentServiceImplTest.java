...
// when(paymentRepository.findByAdminFilters(any(), any(), any(), any(), any(),
// any(), any()))
// .thenReturn(page);
...
// verify(paymentRepository).findByAdminFilters(any(), any(), any(), any(),
// any(), any(), any());
// }
...
// // Given
// Page<PaymentEntity> page = new PageImpl<>(List.of(payment));
// when(paymentRepository.findByCustomerFilters(any(), any(), any(), any(),
// any(), any()))
// .thenReturn(page);
// when(transactionRepository.findByPayment_IdOrderByCreatedAtDesc(paymentId))
// .thenReturn(List.of(transaction));
...
// verify(paymentRepository).findByCustomerFilters(eq(customerId), any(), any(),
// any(), any(),
// any());
// }
...
// // Then
// verify(paymentRepository).findByCustomerFilters(any(), any(), any(), any(),
// any(),
...
// // Given
// filterRequest.setFromDate(java.time.Instant.now().minus(7,
...
// verify(paymentRepository).findByAdminFilters(any(), any(), any(), any(),
// any(), any(), any());
// verify(paymentRepository, never()).findByCustomerFilters(any(), any(), any(),
// any(), any(),
// any());
// }
// }
...
// @Test
// @DisplayName("Should throw PAYMENT_ACCESS_DENIED when customer tries to view
// other's payment")
...
// assertEquals(PaymentErrorCode.PAYMENT_ACCESS_DENIED,
...
// // Given
// Page<PaymentEntity> page = new PageImpl<>(List.of(payment));
// when(paymentRepository.findByAdminTransactionFilters(any(), any(), any(),
// any(), any()))
// .thenReturn(page);
// when(transactionRepository.findByPayment_IdOrderByCreatedAtDesc(paymentId))
// .thenReturn(List.of(transaction));
...
// assertNotNull(result);
// assertEquals(1, result.getContent().size());
// verify(paymentRepository).findByAdminTransactionFilters(any(), any(), any(),
// any(), any());
// }
...
// // Then
// AdminTransactionListResponse.TransactionItem item =
...
// when(paymentRepository.existsByOrder_IdAndStatus(orderId,
// PaymentStatus.PAID))
// .thenReturn(false);
// when(paymentRepository.findFirstByOrder_IdAndStatus(orderId,
...
// when(vnPayService.createPaymentUrl(anyString(), anyLong(), anyString(),
// anyString()))
...
// verify(paymentRepository).findFirstByOrder_IdAndStatus(orderId,
// PaymentStatus.PENDING);
...
// verify(vnPayService).createPaymentUrl(anyString(), anyLong(), anyString(),
// anyString());
// }
...
// assertEquals(PaymentErrorCode.PAYMENT_ALREADY_PAID,
// exception.getErrorCode());
// verify(paymentRepository, never()).save(any());
// verify(transactionRepository, never()).save(any());
// }
...
// @Test
// @DisplayName("Should cancel expired PENDING payment and create new one on
// retry")
...
// @Test
// @DisplayName("Should reject new payment when an active (unexpired) PENDING
// payment exists")
...
// assertEquals(PaymentErrorCode.PAYMENT_PENDING_ACTIVE,
// exception.getErrorCode());
...
// @Test
// @DisplayName("Should reuse existing payUrl when same-method active PENDING
// payment exists")
...
// assertEquals("https://sandbox.vnpayment.vn/existing-link",
// result.paymentUrl());
// assertEquals("PENDING", result.paymentStatus());
// assertEquals(PaymentStatus.PENDING, activePayment.getStatus());
// verify(paymentRepository, never()).save(any());
...
// assertEquals(PaymentErrorCode.PAYMENT_PROVIDER_ERROR,
// exception.getErrorCode());
...
// assertEquals(createPaymentRequest.transactionId(),
// capturedTxn.getVnpTxnRef());
// assertEquals(BigDecimal.valueOf(100.00), capturedTxn.getAmount());
// // TransactionStatus is no longer on TransactionEntity — status is tracked on
// PaymentEntity
...
// doNothing().when(orderTrackingService).sendTrackingUpdate(any(), any(),
// any(), any());
...
// @Test
// @DisplayName("Should process code 24 (customer cancelled) and update payment
// to CANCELLED")
...
// @Test
// @DisplayName("Should throw PAYMENT_NOT_FOUND when transaction does not
// exist")
...
// @Test
// @DisplayName("Should be idempotent: processWebhook does not duplicate a
...
// // Then — transaction carries the gateway IDs; response code/bank live on
// payment
// TransactionEntity capturedTxn = txnCaptor.getValue();
// assertEquals("VNP-123456", capturedTxn.getVnpTransactionNo());
// assertEquals(0, new
// BigDecimal("100000.00").compareTo(capturedTxn.getAmount()));

// // Response codes are now on PaymentEntity
// org.mockito.ArgumentCaptor<PaymentEntity> payCaptor =
...
// @Test
// @DisplayName("Should accept locally-cancelled VNPay transaction when gateway
// says success and no other PAID payment")
...
// @Test
// @DisplayName("Should detect double payment when locally-cancelled VNPay
// transaction succeeds but order already paid")
...
// verify(transactionRepository).save(any(TransactionEntity.class)); // Records
// txn for
// // reconciliation
// verify(paymentRepository, never()).save(any(PaymentEntity.class)); // Does
// NOT change payment
// }
...
// @Test
// @DisplayName("[Code 07] SUSPICIOUS — payment set to PAID with requiresReview
// flag")
...
// @Test
// @DisplayName("[Code 11] EXPIRED — payment set to FAILED, order back to
// PENDING")
...
// @Test
// @DisplayName("[Unknown code] Fallback to UNKNOWN enum — payment set to
// FAILED")
