...
public interface PaymentService {
    ...
    /**
     * Get current payment status for an order (for "Cảm ơn" screen, frontend polling after Online payment).
     * Returns the latest payment for the order.
     * FRANCHISE_ADMIN and STORE_MANAGER can view any order; CUSTOMER can only view their own.
     */
    PaymentStatusResponse getPaymentStatus(UUID orderId, UUID currentUserId, String role);

    /**
     * List all transactions for admin reconciliation (đối soát dòng tiền). Admin/Manager only.
