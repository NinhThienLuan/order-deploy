...
export const getAdminProducts = async ({ page = 0, size = 12, categoryId, search, type } = {}) => {
...
export const createProductVariant = async (productId, payload) => {
  const res = await apiClient.post(`${BASE}/${productId}/variants`, payload);
  return res.data.result;
};

export const updateProductVariant = async (productId, variantId, payload) => {
  const res = await apiClient.put(`${BASE}/${productId}/variants/${variantId}`, payload);
  return res.data.result;
};
...
export const uploadProductImages = async (productId, files, setPrimaryFirst = true) => {
  ...
  const res = await apiClient.post(
    `${BASE}/${productId}/images?setPrimaryFirst=${setPrimaryFirst}`,
    form,
  );
  return res.data.result ?? [];
};
...
export const setPrimaryImage = async (imageId) => {
  const res = await apiClient.patch(`${BASE}/images/${imageId}/primary`);
  return res.data.result;
};
