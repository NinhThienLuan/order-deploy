...
/**
 * Confirm cash payment (Admin/Store Manager only).
 * Used to mark cash payments as confirmed after physical payment is received.
 * 
 * @param {string} paymentId - UUID of the payment
 * @returns {Promise<Object>}
 */
export const confirmCashPayment = async (paymentId) => {
    const res = await apiClient.patch(`/api/v1/payments/${paymentId}/confirm-cash`);
    return res.data.result;
};
